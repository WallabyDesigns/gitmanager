        ...
        <div class="bg-white dark:bg-slate-900 shadow-sm sm:rounded-xl border border-slate-200/60 dark:border-slate-800 p-6">
            <div class="space-y-4">
                <div class="flex flex-wrap items-center justify-between gap-3">
                    ...
                    @if ($latest)
                        @php($latestWarning = $latest->status === 'failed' && str_contains($latest->output_log ?? '', 'stashed changes could not be restored'))
            ...
            <div class="mt-4 space-y-3">
                @forelse ($recent as $update)
                    <div class="rounded-lg border border-slate-200/70 dark:border-slate-800 p-4">
                        @php($updateWarning = $update->status === 'failed' && str_contains($update->output_log ?? '', 'stashed changes could not be restored'))
