...
@if (! $update)
    <p class="mt-3 text-sm text-slate-500 dark:text-slate-400">{{ __('No entries yet.') }}</p>
@else
    <div class="mt-3 min-w-0 rounded-lg border border-slate-200/70 dark:border-slate-800 p-4">
        <div class="flex flex-wrap items-center justify-between gap-2">
            <div>
                ...
                <div class="text-xs text-slate-400 dark:text-slate-500">{{ \App\Support\DateFormatter::forUser($update->started_at, 'M j, Y g:i a', __('Queued')) }}</div>
            </div>
        ...
        @elseif ($logLength > 0)
            ...
            @if (($expandedUpdateId ?? null) === $update->id)
                ...
                @if ($expandedUpdateLogTruncated ?? false)
                    <div class="mt-2 text-[11px] text-amber-500 dark:text-amber-300">{{ __('Showing the latest log tail.') }}</div>
                @endif
            @endif
        @endif
    </div>
