...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
    ...
    <div class="hidden lg:block mt-8 border-t border-slate-800 pt-4 space-y-2">
        ...
        <div class="flex flex-row  space-x-2">
            ...
            @endif
            <button type="button" wire:click="processQueue" wire:loading.attr="disabled" wire:target="processQueue" @disabled($queueUnavailable) class="w-full px-3 py-2 text-xs rounded-md border border-amber-400/50 text-amber-200 hover:text-white hover:border-amber-300 inline-flex items-center justify-center disabled:opacity-60 disabled:cursor-not-allowed">
                @if ($queueBusy)
                    <img src="{{ asset('images/loading.svg') }}" alt="" class="gwm-spinner-visible gwm-spinner w-3 h-3 mr-1" />
                @else
                    <x-loading-spinner target="processQueue" size="w-3 h-3" class="mr-1" />
                @endif
                {{ __('Process Queue') }}
            </button>
        </div>
