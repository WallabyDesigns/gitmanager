        ...
        <div class="flex flex-wrap items-center justify-between gap-3">
            <div class="min-w-0">
                ...
                <div class="mt-2 flex flex-wrap items-center gap-2">
                    ...
                    <span class="rounded-full px-2 py-1 text-xs bg-emerald-500/10 text-emerald-300">
                        {{ __('Running') }} {{ $running }}
                    </span>
                    @if (($stats['stopped'] ?? 0) > 0)
                        <span class="rounded-full px-2 py-1 text-xs bg-amber-500/10 text-amber-300">{{ __('Stopped') }} {{ $stats['stopped'] }}</span>
                    @endif
                    @if ($total > 0)
                        <span class="rounded-full px-2 py-1 text-xs bg-slate-800 text-slate-300">{{ $runPct }}%</span>
                    @endif
                </div>
            </div>
