...
<a href="{{ route('projects.show', $project) }}" class="block rounded-lg border border-slate-800 bg-slate-900 p-4 transition hover:border-indigo-500/60 hover:shadow-sm">
            ...
            @if ($history->count() > 0)
                <div class="hidden sm:flex items-end gap-0.5 h-5">
                    @foreach ($history->take(30) as $check)
                        <span class="w-1.5 rounded-sm {{ $check->status === 'success' ? 'bg-emerald-400' : ($check->status === 'inconclusive' ? 'bg-slate-300' : 'bg-rose-400') }}" style="height: {{ $check->status === 'success' ? '100%' : ($check->status === 'inconclusive' ? '35%' : '50%') }}"></span>
                    @endforeach
                </div>
                @if ($uptimePercent !== null)
                    <span class="text-xs font-medium {{ $uptimePercent >= 95 ? 'text-emerald-600' : ($uptimePercent >= 75 ? 'text-amber-600' : 'text-rose-400') }}">{{ $uptimePercent }}%</span>
                @endif
            @endif
            @if ($project->hasHealthMonitoring())
    ...
    <div class="mt-2 ml-4 flex flex-wrap items-center gap-2 text-xs text-slate-500">
