        ...
        <div class="py-8">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
                    ...
                    <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 5000)"
                         class="flex items-center gap-3 rounded-lg border px-4 py-3 text-sm font-medium
                             {{ $flashType === 'success'
                                 ...
                                 : 'border-rose-200 bg-rose-50 text-rose-800 dark:border-rose-700 dark:bg-rose-950/40 dark:text-rose-300' }}">
                ...
                @if (! $dockerAvailable && $initialSection !== 'templates')
                    <div class="rounded-xl border border-amber-200 dark:border-amber-700/50 bg-amber-50 dark:bg-amber-950/30 p-6 flex items-start gap-4">
                        ...
                        <div>
                            <p class="font-semibold text-amber-900 dark:text-amber-200">{{ __('Docker not detected') }}</p>
                            <p class="mt-1 text-sm text-amber-700 dark:text-amber-300">{{ __('Make sure Docker is installed and running on this server, and that the :var env variable points to the correct binary (default: :default).', ['var' => '<code class="font-mono bg-amber-100 dark:bg-amber-900/50 px-1 rounded">GWM_DOCKER_BINARY</code>', 'default' => '<code class="font-mono bg-amber-100 dark:bg-amber-900/50 px-1 rounded">docker</code>']) }}</p>
                        </div>
                    </div>
                @endif
