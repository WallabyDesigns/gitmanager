    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" wire:click.self="$set('showCreate', false)">
        <div class="w-full max-w-2xl rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-2xl flex flex-col max-h-[90vh]">
            ...
            <div class="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 px-6 py-4 shrink-0">
                <h3 class="font-semibold text-slate-900 dark:text-slate-100">{{ __('New Container') }}</h3>
            ...
            <div class="overflow-y-auto flex-1 p-6 space-y-5">
                ...
                @if ($cliPasteMode)
                    <div class="space-y-3">
                        <p class="text-sm text-slate-700 dark:text-slate-300 font-medium">{{ __('Paste a :command command:', ['command' => '<code class="font-mono text-xs bg-slate-100 dark:bg-slate-800 px-1 rounded">docker run</code>']) }}</p>
                    ...
                    <div class="border-t border-slate-200 dark:border-slate-800 pt-4">
                        <p class="text-xs text-slate-500 dark:text-slate-400">{{ __('Parsed fields will appear below for review before deploying.') }}</p>
                    </div>
                @endif

                {{-- Image --}}
                <div>
                    <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">{{ __('Image') }} <span class="text-rose-500">*</span></label>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">{{ __('Container name') }}</label>
                        <input wire:model="createForm.name" type="text" placeholder="my-app"
                               class="w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 px-3 py-2 text-sm text-slate-900 dark:text-slate-100 focus:border-indigo-400 focus:outline-none">
                    </div>
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">{{ __('Hostname') }}</label>
                ...
                <div>
                    <div class="flex items-center justify-between mb-1.5">
                        <label class="text-sm font-semibold text-slate-700 dark:text-slate-300">{{ __('Port mappings') }}</label>
...
{{-- Env --}}
                <div>
                    <div class="flex items-center justify-between mb-1.5">
                        <label class="text-sm font-semibold text-slate-700 dark:text-slate-300">{{ __('Environment Variables') }}</label>
                        <button wire:click="addCreateField('env')" type="button" class="text-xs text-indigo-600 dark:text-indigo-400 hover:underline">+ {{ __('Add') }}</button>
                    </div>
                </div>
                ...
                <div>
                    <div class="flex items-center justify-between mb-1.5">
                        <label class="text-sm font-semibold text-slate-700 dark:text-slate-300">{{ __('Volume mounts') }}</label>
                        <button wire:click="addCreateField('volumes')" type="button" class="text-xs text-indigo-600 dark:text-indigo-400 hover:underline">+ {{ __('Add') }}</button>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">{{ __('Network') }}</label>
                    ...
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">{{ __('Restart policy') }}</label>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">{{ __('Memory limit') }}</label>
                        <input wire:model="createForm.memory" type="text" placeholder="512m, 1g …"
                               class="w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 px-3 py-2 text-sm text-slate-900 dark:text-slate-100 focus:border-indigo-400 focus:outline-none">
                    </div>
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">{{ __('CPU limit') }}</label>
                ...
                {{-- Command --}}
                <div>
                    <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">{{ __('Command override') }} <span class="text-slate-400 font-normal">({{ __('optional') }})</span></label>
