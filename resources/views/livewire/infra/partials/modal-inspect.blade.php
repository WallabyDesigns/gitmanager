@if ($showInspect)
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" wire:click.self="$set('showInspect', false)">
        <div class="w-full max-w-4xl rounded-2xl border border-slate-700 bg-slate-900 shadow-2xl flex flex-col max-h-[90vh]">
            <div class="flex items-center justify-between border-b border-slate-800 px-6 py-4 shrink-0">
                <h3 class="font-semibold text-slate-100">{{ __('Inspect Details') }}</h3>
                <button wire:click="$set('showInspect', false)" class="text-slate-400 hover:text-slate-200">
            ...
            <div class="overflow-y-auto flex-1 p-6 space-y-4">
                @if ($summaryRows !== [])
                    <div class="rounded-lg border border-slate-800 overflow-hidden">
                        <div class="bg-slate-800/50 px-4 py-2 text-xs uppercase tracking-wide text-slate-400">{{ __('Summary') }}</div>
                        <div class="divide-y divide-slate-800">
                            @foreach ($summaryRows as $row)
                                <div class="px-4 py-2.5 text-sm grid grid-cols-[180px,1fr] gap-3">
                                    <div class="text-slate-400">{{ $row['label'] }}</div>
                                    <div class="text-slate-100 break-all">{{ $renderInspectValue($row['value']) }}</div>
                                </div>
                            @endforeach
                        </div>
                    </div>
                ...
                @foreach ($sections as $section)
                    <div class="rounded-lg border border-slate-800 overflow-hidden">
                        <div class="bg-slate-800/50 px-4 py-2 text-xs uppercase tracking-wide text-slate-400">
                            {{ $section['title'] }}
                        </div>
                        <div class="divide-y divide-slate-800">
                ...
                <details class="rounded-lg border border-slate-800 p-3">
                    <summary class="cursor-pointer text-xs uppercase tracking-wide text-slate-400">{{ __('Raw JSON') }}</summary>
