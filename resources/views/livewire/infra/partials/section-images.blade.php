<div class="flex items-center justify-between">
    <h2 class="text-base font-semibold text-slate-900 dark:text-slate-100">{{ __('Images') }}</h2>
...
@if (count($images) === 0)
    <div class="rounded-xl border border-dashed border-slate-300 dark:border-slate-700 p-12 text-center">
        ...
        <p class="mt-3 text-sm font-medium text-slate-600 dark:text-slate-400">{{ __('No images found locally.') }}</p>
        <button wire:click="$set('showPull', true)" class="mt-4 text-sm text-indigo-600 dark:text-indigo-400 hover:underline">{{ __('Pull an image →') }}</button>
    </div>
@else
                        ...
                        <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-800/30">
                            ...
                            <td class="px-4 py-3">
                                <div class="flex items-center gap-2">
                                    <button wire:click="inspectImage('{{ $imgId }}')" class="text-xs px-2 py-1 rounded border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400 hover:border-indigo-300 hover:text-indigo-600 transition">{{ __('Inspect') }}</button>
                                    <button wire:click="openRetagImageModal('{{ addslashes($retagSource) }}')" class="text-xs px-2 py-1 rounded border border-indigo-400/60 text-indigo-700 hover:text-indigo-900 dark:border-indigo-500/60 dark:text-indigo-300 dark:hover:text-indigo-100 transition">{{ __('Retag') }}</button>
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" wire:click.self="$set('showRetagImage', false)">
        <div class="w-full max-w-md rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-2xl">
            <div class="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 px-6 py-4">
                <h3 class="font-semibold text-slate-900 dark:text-slate-100">{{ __('Retag Image') }}</h3>
            ...
            <div class="p-6 space-y-4">
                <div>
                    <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">{{ __('Source image') }}</label>
                ...
                <div>
                    <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">{{ __('New tag') }}</label>
                ...
                <div class="flex justify-end gap-3">
                    <button wire:click="$set('showRetagImage', false)" class="px-4 py-2 text-sm font-medium text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-100">{{ __('Cancel') }}</button>
...
@if ($showPull)
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" wire:click.self="$set('showPull', false)">
        <div class="w-full max-w-md rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-2xl">
            <div class="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 px-6 py-4">
                <h3 class="font-semibold text-slate-900 dark:text-slate-100">{{ __('Pull Image') }}</h3>
            ...
            <div class="p-6 space-y-4">
                <div>
                    <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">{{ __('Image name') }}</label>
                ...
                <div class="flex justify-end gap-3">
                    <button wire:click="$set('showPull', false)" class="px-4 py-2 text-sm font-medium text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-100">{{ __('Cancel') }}</button>
