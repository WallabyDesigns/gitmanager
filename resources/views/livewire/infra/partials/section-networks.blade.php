<div class="flex items-center justify-between">
    <h2 class="text-base font-semibold text-slate-900 dark:text-slate-100">{{ __('Networks') }}</h2>
...
@if (count($networks) === 0)
    <div class="rounded-xl border border-dashed border-slate-300 dark:border-slate-700 p-12 text-center">
        <p class="text-sm font-medium text-slate-600 dark:text-slate-400">{{ __('No networks found.') }}</p>
    </div>
@else
                        ...
                        <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-800/30">
                            ...
                            <td class="px-4 py-3">
                                <div class="flex items-center gap-2">
                                    <button wire:click="inspectNetwork('{{ $name }}')" class="text-xs px-2 py-1 rounded border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400 hover:border-indigo-300 hover:text-indigo-600 transition">{{ __('Inspect') }}</button>
                                    <button wire:click="openCloneNetworkModal('{{ $name }}', '{{ $net['Driver'] ?? 'bridge' }}')" class="text-xs px-2 py-1 rounded border border-indigo-400/60 text-indigo-700 hover:text-indigo-900 dark:border-indigo-500/60 dark:text-indigo-300 dark:hover:text-indigo-100 transition">{{ __('Edit') }}</button>
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" wire:click.self="$set('showCloneNetwork', false)">
        <div class="w-full max-w-md rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-2xl">
            <div class="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 px-6 py-4">
                <h3 class="font-semibold text-slate-900 dark:text-slate-100">{{ __('Edit Network') }}</h3>
            ...
            <div class="p-6 space-y-4">
                ...
                <div>
                    <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">{{ __('Source network') }}</label>
                ...
                <div>
                    <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">{{ __('New network name') }}</label>
                ...
                <div class="flex justify-end gap-3">
                    <button wire:click="$set('showCloneNetwork', false)" class="px-4 py-2 text-sm font-medium text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-100">{{ __('Cancel') }}</button>
...
@if ($showCreateNetwork)
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" wire:click.self="$set('showCreateNetwork', false)">
        <div class="w-full max-w-md rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-2xl">
            <div class="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 px-6 py-4">
                <h3 class="font-semibold text-slate-900 dark:text-slate-100">{{ __('Create Network') }}</h3>
            ...
            <div class="p-6 space-y-4">
                <div>
                    <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">{{ __('Network Name') }}</label>
                ...
                <div class="flex justify-end gap-3">
                    <button wire:click="$set('showCreateNetwork', false)" class="px-4 py-2 text-sm font-medium text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-100">{{ __('Cancel') }}</button>
