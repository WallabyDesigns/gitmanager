    ...
    @foreach ([
        ...
        @php
            $iconClass = match ($card['color']) {
                'emerald' => 'gwm-stat-icon-success',
                'indigo' => 'gwm-stat-icon-info',
                'violet' => 'gwm-stat-icon-accent',
                default => 'gwm-stat-icon-muted',
            };
        @endphp
        <div class="rounded-xl border border-slate-800 bg-slate-900 p-5">
            <div class="flex items-center justify-between">
                <p class="text-sm text-slate-400">{{ $card['label'] }}</p>
                <span class="gwm-stat-icon {{ $iconClass }}">
                    <svg class="gwm-stat-icon-svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.75">
                        <path stroke-linecap="round" stroke-linejoin="round" d="{{ $card['icon'] }}"/>
                    </svg>
                </span>
            </div>
            <p class="mt-3 text-3xl font-bold text-slate-100">{{ $card['value'] }}</p>
        </div>
    @endforeach
</div>
...
@if (count($containerStats) > 0)
    <div class="rounded-xl border border-slate-800 bg-slate-900 overflow-hidden">
        <div class="px-5 py-4 border-b border-slate-800 flex items-center justify-between">
            <h3 class="font-semibold text-slate-100">{{ __('Container Resource Usage') }}</h3>
            <button wire:click="loadPageData" class="text-xs text-slate-500 hover:text-slate-300 flex items-center gap-1">
                ...
                <thead class="bg-slate-800/50">
                    <tr>
                        @foreach (['Name', 'CPU %', 'Memory', 'Net I/O', 'Block I/O', 'PIDs'] as $h)
                            <th class="px-4 py-2.5 text-xs font-semibold text-slate-400 uppercase tracking-wide">{{ $h }}</th>
                        @endforeach
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-800">
                    @foreach ($containerStats as $stat)
                        ...
                        <tr class="hover:bg-slate-800/30">
                            <td class="px-4 py-3 font-medium text-slate-100">{{ $stat['Name'] ?? '—' }}</td>
                            <td class="px-4 py-3">
                                <div class="flex items-center gap-2">
                                    ...
                                    <span class="text-xs {{ $cpu > 80 ? 'text-rose-600' : ($cpu > 50 ? 'text-amber-600' : 'text-slate-600 text-slate-400') }}">{{ $stat['CPUPerc'] ?? '—' }}</span>
                                </div>
                            </td>
                            <td class="px-4 py-3 text-slate-400">{{ $stat['MemUsage'] ?? '—' }}</td>
                            <td class="px-4 py-3 text-slate-400">{{ $stat['NetIO'] ?? '—' }}</td>
                            <td class="px-4 py-3 text-slate-400">{{ $stat['BlockIO'] ?? '—' }}</td>
                            <td class="px-4 py-3 text-slate-400">{{ $stat['PIDs'] ?? '—' }}</td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
...
@if (count($containers) > 0)
    <div class="rounded-xl border border-slate-800 bg-slate-900 overflow-hidden">
        <div class="px-5 py-4 border-b border-slate-800 flex items-center justify-between">
            <h3 class="font-semibold text-slate-100">{{ __('All Containers') }}</h3>
            <a href="{{ route('infra.containers.section', 'containers') }}" class="text-xs text-indigo-400 hover:underline">{{ __('Manage') }} →</a>
        </div>
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                <thead class="bg-slate-800/50">
                ...
                <tbody class="divide-y divide-slate-800">
                    @foreach ($containers as $c)
                        <tr class="hover:bg-slate-800/30">
                            <td class="px-4 py-3 font-medium text-slate-100">{{ $c['Names'] ?? $c['ID'] ?? '—' }}</td>
                            <td class="px-4 py-3 text-slate-400 font-mono text-xs">{{ Str::limit($c['Image'] ?? '', 40) }}</td>
                            <td class="px-4 py-3">
                            ...
                            <td class="px-4 py-3 text-slate-400 font-mono text-xs">{{ Str::limit($c['Ports'] ?? '', 40) }}</td>
                            <td class="px-4 py-3">
                                @if ($state === 'running')
                                    <button wire:click="stopContainer('{{ $c['ID'] }}')" class="text-xs text-slate-500 hover:text-rose-400">{{ __('Stop') }}</button>
                                @else
                                    <button wire:click="startContainer('{{ $c['ID'] }}')" class="text-xs text-slate-500 hover:text-emerald-400">{{ __('Start') }}</button>
                                @endif
                            </td>
                        </tr>
                    @endforeach
