@if (! $swarmInfo['active'])
    ...
    <div class="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-12 text-center space-y-4">
        ...
        <div>
            <h3 class="text-base font-semibold text-slate-900 dark:text-slate-100">Docker Swarm is not active</h3>
            <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Initialise a swarm on this node to enable multi-container orchestration across hosts.</p>
        </div>
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" wire:click.self="$set('showSwarmInit', false)">
            <div class="w-full max-w-md rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-2xl">
                <div class="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 px-6 py-4">
                    <h3 class="font-semibold text-slate-900 dark:text-slate-100">Initialise Docker Swarm</h3>
                ...
                <div class="p-6 space-y-4">
                    <div>
                        <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">Advertise address <span class="text-slate-400 font-normal">(optional)</span></label>
                        ...
                        <p class="mt-1.5 text-xs text-slate-500">Leave blank to let Docker choose automatically. Required when this node has multiple IP addresses.</p>
                    </div>
                    <div class="flex justify-end gap-3">
                        <button wire:click="$set('showSwarmInit', false)" class="px-4 py-2 text-sm font-medium text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-100">Cancel</button>
                        <button wire:click="initSwarm" class="rounded-lg bg-indigo-600 hover:bg-indigo-500 px-4 py-2 text-sm font-semibold text-white shadow-sm transition">Initialise</button>
                    </div>
                </div>
            </div>
        </div>
...
@else
    ...
    @if (count($swarmNodes) > 0)
        <div class="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 overflow-hidden">
            <div class="px-5 py-4 border-b border-slate-200 dark:border-slate-800">
                <h3 class="font-semibold text-slate-900 dark:text-slate-100">Nodes</h3>
            </div>
            <div class="overflow-x-auto">
                <table class="w-full text-sm">
                    <thead class="bg-slate-50 dark:bg-slate-800/50">
                        <tr>
                            @foreach (['Hostname', 'Status', 'Availability', 'Role', 'Engine'] as $h)
                        ...
                        @foreach ($swarmNodes as $node)
                            <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-800/30">
                                ...
                                <td class="px-4 py-3">
                                    ...
                                    <span class="inline-flex rounded-md px-2 py-0.5 text-xs font-medium {{ $role === 'Manager' ? 'bg-indigo-100 text-indigo-700 dark:bg-indigo-500/10 dark:text-indigo-400' : 'bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-400' }}">{{ $role }}</span>
                                </td>
    ...
    @if (count($swarmServices) > 0)
        <div class="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 overflow-hidden">
            <div class="px-5 py-4 border-b border-slate-200 dark:border-slate-800">
                <h3 class="font-semibold text-slate-900 dark:text-slate-100">Services</h3>
            </div>
            <div class="overflow-x-auto">
                <table class="w-full text-sm">
                    <thead class="bg-slate-50 dark:bg-slate-800/50">
                            ...
                            <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-800/30">
                                ...
                                <td class="px-4 py-3">
                                    <button wire:click="openScaleModal('{{ $svcName }}', {{ $desired }})"
                                            class="text-xs px-2 py-1 rounded border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400 hover:border-indigo-300 hover:text-indigo-600 transition">Scale</button>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
    ...
    @if ($showScale)
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" wire:click.self="$set('showScale', false)">
            <div class="w-full max-w-sm rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-2xl">
                <div class="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 px-6 py-4">
                    <h3 class="font-semibold text-slate-900 dark:text-slate-100">Scale Service</h3>
                    <button wire:click="$set('showScale', false)" class="text-slate-400 hover:text-slate-600 dark:hover:text-slate-200">
                        <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/></svg>
                    </button>
                </div>
                <div class="p-6 space-y-4">
                    <p class="text-sm text-slate-700 dark:text-slate-300">Service: <span class="font-semibold">{{ $scaleService }}</span></p>
                    <div>
                        <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">Replicas</label>
                        <input wire:model="scaleReplicas" type="number" min="0" max="100"
                               class="w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 px-3 py-2 text-sm text-slate-900 dark:text-slate-100 focus:border-indigo-400 focus:outline-none">
                    </div>
                    <div class="flex justify-end gap-3">
                        <button wire:click="$set('showScale', false)" class="px-4 py-2 text-sm font-medium text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-100">Cancel</button>
                        <button wire:click="scaleService" class="rounded-lg bg-indigo-600 hover:bg-indigo-500 px-4 py-2 text-sm font-semibold text-white shadow-sm transition">Apply</button>
                    </div>
                </div>
            </div>
        </div>
