<div class="flex items-center justify-between">
    <h2 class="text-base font-semibold text-slate-100">{{ __('Volumes') }}</h2>
...
@if (count($volumes) === 0)
    <div class="rounded-xl border border-dashed border-slate-700 p-12 text-center">
        ...
        <p class="mt-3 text-sm font-medium text-slate-400">{{ __('No volumes found.') }}</p>
        <button wire:click="$set('showCreateVolume', true)" class="mt-4 text-sm text-indigo-400 hover:underline">{{ __('Create a volume →') }}</button>
    </div>
@else
                ...
                <tbody class="divide-y divide-slate-800">
                    @foreach ($volumes as $vol)
                        ...
                        <tr class="hover:bg-slate-800/30">
                            ...
                            <td class="px-4 py-3">
                                <span class="inline-flex rounded-md bg-slate-800 px-2 py-0.5 text-xs font-medium text-slate-300">{{ $vol['Driver'] ?? 'local' }}</span>
                            </td>
                            <td class="px-4 py-3 font-mono text-xs text-slate-400">{{ Str::limit($vol['Mountpoint'] ?? '—', 50) }}</td>
                            <td class="px-4 py-3">
                                <div class="flex items-center gap-2">
                                    <button wire:click="inspectVolume('{{ $name }}')" class="text-xs px-2 py-1 rounded border border-slate-700 text-slate-400 hover:border-indigo-300 hover:text-indigo-600 transition">{{ __('Inspect') }}</button>
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" wire:click.self="$set('showCreateVolume', false)">
        <div class="w-full max-w-md rounded-2xl border border-slate-700 bg-slate-900 shadow-2xl">
            <div class="flex items-center justify-between border-b border-slate-800 px-6 py-4">
                <h3 class="font-semibold text-slate-100">{{ __('Create Volume') }}</h3>
                <button wire:click="$set('showCreateVolume', false)" class="text-slate-400 hover:text-slate-200">
                    <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/></svg>
                </button>
            </div>
            <div class="p-6 space-y-4">
                <div>
                    <label class="block text-sm font-medium text-slate-300 mb-1.5">{{ __('Volume Name') }}</label>
                ...
                <div class="flex justify-end gap-3">
                    <button wire:click="$set('showCreateVolume', false)" class="px-4 py-2 text-sm font-medium text-slate-400 hover:text-slate-100">{{ __('Cancel') }}</button>
