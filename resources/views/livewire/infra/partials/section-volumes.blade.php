<div class="flex items-center justify-between">
    <h2 class="text-base font-semibold text-slate-900 dark:text-slate-100">Volumes</h2>
...
@if (count($volumes) === 0)
    <div class="rounded-xl border border-dashed border-slate-300 dark:border-slate-700 p-12 text-center">
        ...
        <p class="mt-3 text-sm font-medium text-slate-600 dark:text-slate-400">No volumes found.</p>
        <button wire:click="$set('showCreateVolume', true)" class="mt-4 text-sm text-indigo-600 dark:text-indigo-400 hover:underline">Create a volume →</button>
    </div>
@else
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                <thead class="bg-slate-50 dark:bg-slate-800/50">
                    <tr>
                        @foreach (['Name', 'Driver', 'Mountpoint', 'Actions'] as $h)
                        ...
                        <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-800/30">
                            ...
                            <td class="px-4 py-3">
                                <div class="flex items-center gap-2">
                                    <button wire:click="inspectVolume('{{ $name }}')" class="text-xs px-2 py-1 rounded border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400 hover:border-indigo-300 hover:text-indigo-600 transition">Inspect</button>
                                    <button wire:click="removeVolume('{{ $name }}')"
                                            wire:confirm="Remove volume {{ $name }}? Any data stored in it will be lost."
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" wire:click.self="$set('showCreateVolume', false)">
        <div class="w-full max-w-md rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-2xl">
            <div class="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 px-6 py-4">
                <h3 class="font-semibold text-slate-900 dark:text-slate-100">Create Volume</h3>
            ...
            <div class="p-6 space-y-4">
                <div>
                    <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">Volume name</label>
                ...
                <div class="flex justify-end gap-3">
                    <button wire:click="$set('showCreateVolume', false)" class="px-4 py-2 text-sm font-medium text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-100">Cancel</button>
                    <button wire:click="createVolume" class="rounded-lg bg-indigo-600 hover:bg-indigo-500 px-4 py-2 text-sm font-semibold text-white shadow-sm transition">Create</button>
                </div>
            </div>
        </div>
    </div>
