...
<div>
        ...
        <div class="sm:hidden relative" x-data="{ open: false }" @click.outside="open = false" @keydown.escape="open = false">
            ...
            >
                <span class="flex items-center gap-2">
                    @if ($isKubernetesActive)
                        <svg class="h-4 w-4 text-slate-500 dark:text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.75" aria-hidden="true">
            ...
            >
                <a
                    ...
                    class="flex items-center gap-2 px-4 py-3 text-sm transition {{ $isContainersActive ? 'bg-indigo-50 dark:bg-indigo-500/10 text-indigo-700 dark:text-indigo-300 font-medium' : 'text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-800' }}"
                    role="menuitem"
                >
                ...
                <a
                    ...
                    class="flex items-center gap-2 px-4 py-3 text-sm transition border-t border-slate-100 dark:border-slate-800 {{ $isKubernetesActive ? 'bg-indigo-50 dark:bg-indigo-500/10 text-indigo-700 dark:text-indigo-300 font-medium' : 'text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-800' }}"
                    role="menuitem"
                >
                    <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.75" aria-hidden="true">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M8.288 15.038a5.25 5.25 0 017.424 0M5.106 11.856c3.807-3.808 9.98-3.808 13.788 0M1.924 8.674c5.565-5.565 14.587-5.565 20.152 0M12.53 18.22l-.53.53-.53-.53a.75.75 0 011.06 0z" />
    ...
    <div class="{{ $showMobileDropdown ? 'hidden sm:flex' : 'flex' }} flex-wrap gap-1 border-b border-slate-200/70 dark:border-slate-800">
        @if ($hasContainers)
