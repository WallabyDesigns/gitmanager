<div class="border-b border-slate-800 mb-6">
    <nav class="flex gap-1" aria-label="{{ __('Processes navigation') }}">
        ...
        <a href="{{ route('processes.active') }}"
           class="px-4 py-2.5 text-sm border-b-2 -mb-px {{ request()->routeIs('processes.active') ? 'border-indigo-500 text-white' : 'border-transparent text-slate-400 hover:text-slate-200' }}">
            {{ __('Active Processes') }}
        </a>
    </nav>
</div>
