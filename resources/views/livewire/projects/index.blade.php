...
<div class="py-10" wire:init="refreshHealth" wire:poll.60s="refreshHealth">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="grid gap-6 lg:grid-cols-[260px,1fr]">
            ...
            <div class="space-y-6">
                @if ($showSchedulerNotice && $queueEnabled && ! $schedulerHealthy)
                    <div class="rounded-xl border border-amber-300/60 bg-amber-500/10 p-4 text-sm text-amber-200">
                        <div class="text-xs uppercase tracking-wide text-amber-300">Scheduler not detected</div>
                        <div class="mt-1 text-xs text-amber-100">
                            Queued tasks wait until the scheduler heartbeat resumes.
                            @if ($lastHeartbeat)
                        ...
                        <div class="mt-3">
                            @if ($isAdmin)
                                <a href="{{ route('system.scheduler') }}" class="inline-flex items-center justify-center rounded-md border border-amber-300/60 px-3 py-1.5 text-xs font-semibold text-amber-100 hover:border-white hover:text-white">
                ...
                @if (\Illuminate\Support\Facades\Route::has('infra.containers'))
                    <livewire:infra.container-stats />
                @endif
