<form wire:submit.prevent="{{ $submitAction }}" class="mt-6 space-y-6">
    <div class="grid gap-4 sm:grid-cols-2">
        ...
        <div class="sm:col-span-2 grid gap-4">
            <div class="rounded-lg border border-slate-800 p-4">
                ...
                <p class="mt-1 text-xs text-slate-400">{{ __('Paste an optional .env file to seed on the next deployment. The file is only created when missing, so existing configs are preserved.') }}</p>
            ...
            <div class="rounded-lg border border-slate-800 p-4">
                ...
                <p class="mt-1 text-xs text-slate-400">{{ __('Paste optional Apache rules to seed on the next deployment. Defaults only apply when the file is missing.') }}</p>
            ...
            <div class="rounded-lg border border-slate-800 p-4 space-y-3">
                ...
                <div class="grid gap-3 sm:grid-cols-2">
                    <div>
                        ...
                        <select id="ftp_account_id" class="mt-1 block w-full rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500 border-slate-700 bg-slate-900 text-slate-100" wire:model.live="form.ftp_account_id" {{ ($form['ftp_enabled'] ?? false) ? '' : 'disabled' }}>
                ...
                <div class="flex flex-wrap items-center gap-3">
                    <button type="button" wire:click="testFtpConnection" class="px-3 py-2 text-xs rounded-md border border-slate-700 text-slate-300 hover:text-slate-100" {{ ($form['ftp_enabled'] ?? false) ? '' : 'disabled' }}>
                        {{ __('Test Connection') }}
                    </button>
                ...
                @if (($form['ftp_enabled'] ?? false) && ! ($form['ssh_enabled'] ?? false) && in_array($form['project_type'] ?? 'laravel', ['laravel', '']))
                    <div class="rounded-md border border-amber-500/30 bg-amber-500/10 px-3 py-2 text-xs text-amber-300">
    ...
    <div class="flex flex-wrap items-center gap-3">
        ...
        @if (! empty($cancelUrl))
            <a href="{{ $cancelUrl }}" class="text-sm text-slate-400 hover:text-slate-200">
                {{ __('Cancel') }}
            </a>
        @endif
    </div>
