        ...
        @if (($envSeedPending ?? false) || ($htaccessSeedPending ?? false))
            <div class="mt-2 flex flex-wrap items-center gap-2 text-xs">
                @if ($envSeedPending ?? false)
                    <span class="px-2 py-1 rounded-full bg-amber-100 text-amber-700 dark:bg-amber-500/10 dark:text-amber-300 uppercase tracking-wide">Env seed pending</span>
                @endif
                @if ($htaccessSeedPending ?? false)
                    <span class="px-2 py-1 rounded-full bg-amber-100 text-amber-700 dark:bg-amber-500/10 dark:text-amber-300 uppercase tracking-wide">.htaccess seed pending</span>
                @endif
            </div>
        @endif
