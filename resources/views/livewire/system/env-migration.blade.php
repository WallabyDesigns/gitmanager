<div class="min-h-screen bg-slate-950 flex items-center justify-center p-4">
    <div class="w-full max-w-2xl">
        ...
        <div class="mb-8 text-center">
            ...
            <h1 class="text-xl font-bold text-white">{{ __('Environment Setup') }}</h1>
            <p class="mt-1 text-sm text-slate-400">{{ __('A few new configuration values were added in this update.') }}</p>
        </div>
        ...
        @if ($step === 1)
            <div class="rounded-2xl border border-slate-800 bg-slate-900 overflow-hidden">
                <div class="px-6 py-5 border-b border-slate-800">
                    <h2 class="text-sm font-semibold text-white">{{ __('Missing Environment Variables') }}</h2>
        ...
        @if ($step === 2)
            <div class="rounded-2xl border border-emerald-800/50 bg-emerald-500/5 overflow-hidden text-center">
                <div class="px-6 py-10 space-y-4">
                    ...
                    <div>
                        <h2 class="text-lg font-semibold text-white">{{ __('Setup complete') }}</h2>
                        <p class="mt-1 text-sm text-slate-400">{{ __('Your environment is up to date. You can adjust any values later in :path.', ['path' => '<strong class="text-slate-200">System → Environment Config</strong>']) }}</p>
                    </div>
