<div class="gwm-system-page py-10" wire:init="loadData">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="{{ $securityShell ? 'space-y-6' : 'grid gap-6 lg:grid-cols-[260px,1fr]' }}">
            @if ($securityShell)
                @include('livewire.security.partials.tabs', ['securityTab' => 'settings'])
            @else
            ...
            <div class="gwm-system-content min-w-0 space-y-6">
                ...
                @if ($settingsSection === 'audits')
                    @include('livewire.security.partials.settings-content')
                @endif
                ...
                @if (! in_array($settingsSection, ['environment', 'scheduler', 'diagnostics', 'node'], true))
                <div class="flex flex-wrap items-center gap-3" x-data="{ saved: false, timer: null }" x-on:settings-saved.window="
                    saved = true;
                    clearTimeout(timer);
                    timer = setTimeout(() => saved = false, 2000);
