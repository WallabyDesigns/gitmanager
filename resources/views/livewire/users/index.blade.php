        ...
        <div >
            @if ($tab === 'create')
                <div class="bg-white dark:bg-slate-900 shadow-sm sm:rounded-xl border border-slate-200/60 dark:border-slate-800 p-6 space-y-6">
                    <h3 class="text-lg font-semibold text-slate-900 dark:text-slate-100">{{ __('Create User') }}</h3>
                    <p class="text-sm text-slate-500 dark:text-slate-400">{{ __('Generate accounts for teammates. New users must change their password on first login.') }}</p>
                    <div class="mt-4 grid gap-4 sm:grid-cols-2">
                        ...
                        <div class="flex items-center gap-2 pt-6">
                            ...
                            <label for="user_force_change" class="text-sm text-slate-600 dark:text-slate-300">{{ __('Require password change on first login') }}</label>
                        </div>
                    </div>
                        ...
                        <div class="rounded-lg border border-slate-200/70 dark:border-slate-800 bg-white dark:bg-slate-900 p-4">
                            <div class="flex flex-wrap items-center justify-between gap-3">
                                ...
                                <div class="flex flex-wrap gap-2 text-xs">
                                    ...
                                    @endif
                                    <span class="px-2 py-1 rounded-full bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-300">{{ __('Joined :date', ['date' => \App\Support\DateFormatter::forUser($user->created_at, 'M j, Y')]) }}</span>
                                </div>
                            </div>
