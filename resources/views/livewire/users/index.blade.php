        ...
        <div >
            @if ($tab === 'create')
                <div class="bg-slate-900 shadow-sm sm:rounded-xl border border-slate-800 p-6 space-y-6">
                    <h3 class="text-lg font-semibold text-slate-100">{{ __('Create User') }}</h3>
                    <p class="text-sm text-slate-400">{{ __('Generate accounts for teammates. New users must change their password on first login.') }}</p>
                    <div class="mt-4 grid gap-4 sm:grid-cols-2">
                        ...
                        <div class="flex items-center gap-2 pt-6">
                            ...
                            <label for="user_force_change" class="text-sm text-slate-300">{{ __('Require password change on first login') }}</label>
                        </div>
                    </div>
                    <div class="mt-4 flex flex-wrap gap-3">
                        <button type="button" wire:click="createUser" class="px-4 py-2 rounded-md hover:bg-slate-700 bg-slate-100 text-slate-900">
                            {{ __('Create User') }}
                        </button>
                    </div>
                    @if ($generatedPassword)
                        <div class="mt-4 rounded-lg border border-amber-300/60 bg-amber-500/10 p-4 text-sm text-amber-200">
                            {{ __('Temporary password for user # :id: :password', ['id' => $generatedUserId, 'password' => $generatedPassword]) }}
                        </div>
                    @endif
                </div>
            @endif
                            ...
                            <div class="flex flex-wrap items-center justify-between gap-3">
                                ...
                                <div class="flex flex-wrap gap-2 text-xs">
                                    @if ($user->must_change_password)
                                        <span class="px-2 py-1 rounded-full bg-amber-500/20 text-amber-200">{{ __('Password change required') }}</span>
                                    @endif
                                    <span class="px-2 py-1 rounded-full bg-slate-800 text-slate-300">{{ __('Joined :date', ['date' => \App\Support\DateFormatter::forUser($user->created_at, 'M j, Y')]) }}</span>
                                </div>
                            </div>
                            <div class="mt-3 flex flex-wrap gap-3 text-sm">
                                <button type="button" wire:click="sendPasswordReset({{ $user->id }})" class="text-indigo-300">
                                    {{ __('Send reset link') }}
                                </button>
                                <button type="button" wire:click="resetPassword({{ $user->id }})" class="text-rose-500">
                                    {{ __('Generate temp password') }}
                                </button>
                            </div>
                        </div>
                    @empty
