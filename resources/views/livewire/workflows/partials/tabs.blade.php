<div>
    ...
    <div class="sm:hidden relative" x-data="{ open: false }" @click.outside="open = false" @keydown.escape="open = false">
        ...
        >
            ...
            <button
                ...
                @click="open = false"
                class="flex w-full items-center px-4 py-3 text-sm transition border-t border-slate-800 {{ $tab === 'form' ? 'bg-indigo-500/10 font-medium' : 'text-slate-200 hover:bg-slate-800' }}"
                role="menuitem"
            >
                {{ __('Create Workflow') }}
            </button>
            <button
                ...
                @click="open = false"
                class="flex w-full items-center px-4 py-3 text-sm transition border-t border-slate-800 {{ $tab === 'test' ? 'bg-indigo-500/10 font-medium' : 'text-slate-200 hover:bg-slate-800' }}"
                role="menuitem"
            >
                {{ __('Test Delivery') }}
            </button>
    ...
    <div class="hidden sm:flex flex-wrap gap-2 border-b border-slate-800">
        ...
        <button type="button"
                wire:click="setTab('form')"
                class="px-3 py-2 text-sm border-b-2 {{ $tab === 'form' ? 'border-indigo-500' : 'border-transparent text-slate-400 hover:text-slate-200' }}">
            {{ __('Create Workflow') }}
        </button>
        <button type="button"
                wire:click="setTab('test')"
                class="px-3 py-2 text-sm border-b-2 {{ $tab === 'test' ? 'border-indigo-500' : 'border-transparent text-slate-400 hover:text-slate-200' }}">
            {{ __('Test Delivery') }}
        </button>
    </div>
</div>
